...
public Connection getdbconwithoracle()
{
	...
	try {
		...
		con=ods.getConnection("nishant","icsd");
...
public clsticket getseatsbookedbyidandtime(String strmid, String strshowtime)
{
	clsticket objci=null;
	
	
	Connection con=getdbconwithoracle();
	try
	{
		PreparedStatement stmt= con.prepareStatement("select goldseatsbooked,silverseatsbooked from tblseatsbooked  where movieid=? and slotofshow=?");
		stmt.setString(1,strmid);
		...
		if(rset.next())
		{
			String strgoldseats,strsilverbooked;
			strgoldseats=rset.getString("goldseatsbooked");
			strsilverbooked=rset.getString("silverseatsbooked");
			
			objci=new clsticket(strgoldseats,strsilverbooked);
		}
		else
		{
			JOptionPane.showMessageDialog(null,"no record found");
